...
public class Steuerung {
    ...
    private Spielfeld dasSpielfeld = new Spielfeld(this);
    private Zelle[][] dieZellen = new Zelle[dasSpielfeld.BREITE][dasSpielfeld.HOEHE];
    private int[][] dasMinenfeld = new int[dasSpielfeld.BREITE][dasSpielfeld.HOEHE];
    private int[][] dasMinenfeldMitRand = new int[dasSpielfeld.BREITE + 2][dasSpielfeld.HOEHE + 2];
    private int[][] dasGefahrenfeld = new int[dasMinenfeld.length][dasMinenfeld[0].length];
    ...
    public void gewonnen() {
        ...
        if (b) {
            JOptionPane.showMessageDialog(null, "VICTORY", "Minesweeper", JOptionPane.INFORMATION_MESSAGE);
            dasSpielfeld.dispose();
        }
    }

    public void deckeAuf(int x, int y) {
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                if (istZelle(x + i, y + j))
                    dieZellen[x + i][y + j].deckeAuf();
            }
        }
    }

    private boolean istZelle(int x, int y) {
        if (x >= dieZellen.length || y >= dieZellen[0].length || x < 0 || y < 0) {
            return false;
        }
        return true;
    }
